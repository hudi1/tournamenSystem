...
public class AdvancedRoundRobinSchedule implements RoundRobinSchedule {
    ...
    public AdvancedRoundRobinSchedule(final Tournament tournament, final Groups group,
            final List<PlayerResult> finalPlayers) {
        this.tournament = tournament;
        this.group = group;
        this.finalPlayers = finalPlayers;
        this.playerByGroup = getPlayersByGroup();
    }
    ...
    private void addGroupRound() {
        if (playerByGroup.size() % 2 == 0) {
            ...
            for (int i = 0; i < playerByGroup.size() / 2; i++) {
                rounds.add(new Round(playerByGroup.get(i), playerByGroup.get(playerByGroup.size() - i - 1)));
            }
    ...
    private LinkedList<List<PlayerResult>> getPlayersByGroup() {
        LinkedList<List<PlayerResult>> playerByGroup = new LinkedList<List<PlayerResult>>();

        System.out.println(tournamentService);
        ...
        for (int i = 0; i < basicGroups.size(); i++) {
            ...
            List<PlayerResult> players = tournamentService.getPlayerResultInGroup(new PlayerResult()
                    ._setGroup(basicGroups.get(i)));
    ...
    public class Round {
        ...
        public Round(List<PlayerResult> playerResut1, List<PlayerResult> playerResult2) {
